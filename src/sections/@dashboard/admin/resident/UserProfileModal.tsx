...
export default function UserProfileModal({
    ...
    const fetchUserProfileData = async (userId: string) => {
        try {
            const response = await httpClient.userService.getUserProfileByUseridForAdmin(userId);
            if (response) {
    ...
    return (
        ...
        <DialogContent dividers>
            ...
            <Box>
                ...
                <Grid container spacing={2}>
                    <Grid item xs={12} sm={6}>
                        ...
                        <b>Full Name:</b> {userProfile?.user?.lastName} {userProfile?.user?.firstName}
                        </Typography>
                    </Grid>
                    <Grid item xs={12} sm={6}>
                        <Typography variant="body2">
                        <b> Date of birth:</b> {fDate(userProfile?.user?.dateOfBirth, "dd/MM/yyyy")}
                        </Typography>
                    </Grid>
                    <Grid item xs={12} sm={6}>
                        <Typography variant="body2">
                        <b>Gender:</b> {getGenderDescription(userProfile?.user?.gender)}
                        </Typography>
                    </Grid>
                    <Grid item xs={12} sm={6}>
                        <Typography variant="body2">
                ...
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} sm={6} key={index}>
                        <Box sx={{ mb: 2 }}>
                            ...
                            <Typography variant="body2" sx={{ mb: 1 }}>
                                <b>Email:</b> {guardian?.email != "" ? guardian?.email : "- -"}
                            </Typography>
                            <Typography variant="body2">
            ...
            <Box>
                ...
                <Typography variant="body2" sx={{ mb: 1 }}>
                    <b>Insurance Number:</b> {userProfile?.healthInsurance?.insuranceCardNumber}
                </Typography>
                <Typography variant="body2" sx={{ mb: 1 }}>
                    <b>Expiration date:</b> {fDate(userProfile?.healthInsurance?.expirationDate, "dd/MM/yyyy")}
                </Typography>
                <Typography variant="body2" sx={{ mb: 1 }}>
                    <b>Registered hospital:</b> {userProfile?.healthInsurance?.registeredHospital}
                </Typography>
            </Box>
            ...
            <Box>
                ...
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} sm={6}>
                        <Typography variant="body2">
                            <b>Status:</b> <ContractStatusTag status={userProfile?.contract?.status}/>
                        </Typography>
                    </Grid>
                    <Grid item xs={12} sm={6}>
                        <Typography variant="body2">
                    ...
                    <Grid item xs={12} sm={6}>
                        <Typography variant="body2">
                            <b>End Date:</b> {fDate(userProfile?.contract?.endDate, "dd/MM/yyyy")}
                        </Typography>
                    </Grid>
                    <Grid item xs={12} sm={6}>
                        <Typography variant="body2">
                            <b>Room number:</b> {userProfile?.contract?.roomNumber}
                        </Typography>
                    </Grid>
                    <Grid item xs={12} sm={6}>
                        <Typography variant="body2">
                    ...
                    <Grid item xs={12} sm={6}>
                        <Typography variant="body2">
                            <b>Room type:</b> {userProfile?.contract?.roomTypeName}
                        </Typography>
                    </Grid>
                </Grid>                
            </Box>
